// @flow
import type { InputSchema, FormSchema } from '../types/formTypes';

export const validateForm = (schema: FormSchema) => {
  let flag = false;
  for (let key in schema) {
    if (
      schema[key].required
        ? schema[key].value.length === 0 || schema[key].error !== ''
        : schema[key].error !== ''
    ) {
      flag = true;
    }
...
export const getFormData = (schema: FormSchema) => {
  let data = {};
  for (let key in schema) {
...
export const populateErrors = (schema: FormSchema) => {
  for (let key in schema) {
    let newVal = validateInput({ ...schema[key] });
    schema = { ...schema, [key]: newVal };
  }
  return schema;
};

export const setInitialSchema = (schema: FormSchema) => {
  for (let key in schema) {
    schema[key]['name'] = key;
  }
  return schema;
